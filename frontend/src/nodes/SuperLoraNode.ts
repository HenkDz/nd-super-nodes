...
class SuperLoraHeaderWidget extends SuperLoraBaseWidget {
  ...
  onSaveTemplateDown = (event: any, pos: any, node: any): boolean => {
    SuperLoraNode.showInlineText(event, 'My LoRA Set', (templateName: string) => {
      if (templateName && templateName.trim()) {
        const configs = SuperLoraNode.getLoraConfigs(node);
        SuperLoraNode.templateService.saveTemplate(templateName.trim(), configs)
          .then(success => {
            if (success) {
...
export class SuperLoraNode {
  ...
  private static loraService: LoraService;
  ...
  static async showLoraSelector(node: any, widget?: SuperLoraWidget, event?: any): Promise<void> {
    try {
      // Get actual LoRAs from ComfyUI
      let availableLoras: string[] = [];
      
      try {
        // Try to get LoRAs from ComfyUI's API
        const response = await fetch('/object_info');
        const objectInfo = await response.json();
        
        // Look for LoRA loader node info to get available LoRAs
      ...
      const searchItem = {
        ...
        callback: () => {
          ...
          if (searchTerm && searchTerm.trim()) {
            const filtered = unusedLoras.filter(lora => 
      ...
      // Create context menu with search first
      const menuItems = [searchItem, null];
      
      // Add first few LoRAs directly
      ...
      // Add option to see more if there are many LoRAs
  ...
  static async showLoadTemplateDialog(node: any, event?: any): Promise<void> {
    try {
      ...
      new LiteGraph.ContextMenu(menuItems, { title: 'Load Template', event: event });
    } catch (error) {
   ...
   */
  private static showToast(message: string, type: 'success' | 'warning' | 'error' = 'success'): void {
